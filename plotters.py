...
def eig_vals_plot(pts, r, eps_list, fig=None, show=True):
    ...
    for eps in eps_list:
        Qeps_list = Q_eps(pts, r=r, eps=eps, load_cached=True)
        Qeps = Qeps_list[-1]
        Q_eigVals, _  = computeQ_eigVals(Qeps, r=r, eps=eps, k=10, load_cached=True)
...
def plot_eigenfuncs_at_times(pts, img_shape,  t_vec, r, eps_list, cmap=None, show=True):
    m = pts.shape[0]
    pts_shape = (m,m)
...
def cluster_2_labels(pts, img_shape, t_vec, r, eps_list, cmap=None, show=True):
    m = pts.shape[0]
    pts_shape = (m,m)
    for eps in eps_list:
        Qeps_list = Q_eps(pts, r=r, eps=eps, load_cached=True)
